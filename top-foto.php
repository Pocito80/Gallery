...
mysqli_query($conn, "SET collation_connection = utf8_polish_ci");
  ...
  $t = mysqli_query ($conn, "SELECT AVG(zdjecia_oceny.ocena), zdjecia_oceny.id_zdjecia, zdjecia.id_album FROM `zdjecia_oceny` INNER JOIN zdjecia ON zdjecia.id=zdjecia_oceny.id_zdjecia GROUP BY (zdjecia_oceny.id_zdjecia) ORDER BY AVG(zdjecia_oceny.ocena) DESC limit 20;");



  while($row1=mysqli_fetch_array($t)){
    array_push($tytulAZ,$row1['id_zdjecia']);
    array_push($zdjeciaID,$row1['id_album']);
    array_push($ocena,$row1['AVG(zdjecia_oceny.ocena)']);
    $k=$k+1;
  }
...
echo'<span class="tooltiptext">'.$row8['tytul'].'<br>'.$ocena[$b].'<br>'.$row8['login']. '</span>
