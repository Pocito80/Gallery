...
mysqli_query($conn, "SET collation_connection = utf8_polish_ci");
    ...
    echo'<p class="title is-1 has-text-primary-light ">Czy na pewno chcesz ocenić to zdjęcie na '.$o.'?</p>
    <form  method="post">
...
<input type="hidden" name="answer" value="'.$o.'" />
    ...
    if (isset($_POST["Submit7"])) {
        ...
        $q = "INSERT INTO zdjecia_oceny (`id_zdjecia`,`id_uzytkownika`,`ocena`) VALUES ('$p','$id','$o')";
        if ($conn->query($q) === TRUE) {
            ob_end_clean();
            echo '<p class="title is-1 has-text-primary-light ">Zdjęcie oceniono pomyślnie</p>
        ...
        <input type="hidden" name="answer" value="'.$o.'" />
            ...
            ob_end_clean();
            echo '<p class="title is-1 has-text-primary-light ">Ocenianie nie powiodło się, spróbuj ponownie później</p>
            <form action="foto.php"  method="post">
            <input type="hidden" name="idzdjecie" value="'.$p.'" />
        <input type="hidden" name="idalbum" value="'.$a.'" />
        <input type="hidden" name="idz" value="'.$p.'" />
